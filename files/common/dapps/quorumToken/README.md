...
The `dapps/besuToken` folder is this structured in this manner (only relevant paths shown):

```
besuToken
├── hardhat.config.ts       // hardhat network config
├── contracts               // sample contracts of which we use the BesuToken.sol
...
npx hardhat run ./scripts/deploy_besutoken.ts --network quickstart
```
